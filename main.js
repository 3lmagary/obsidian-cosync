...
var CoSyncPlugin = class extends import_obsidian.Plugin {
  ...
  startPeriodicSync() {
    ...
    if (this.settings.enableBackgroundSync && this.settings.syncInterval > 0) {
      this.syncTimer = setInterval(() => this.syncVault(), this.settings.syncInterval * 1e3);
    }
  }
  onunload() {
  ...
  async updateServerDocumentTitle(docId, newPath) {
  ...
  async saveBaseText(docId, content) {
    try {
  ...
  async markDocumentSynced(docId, content, hash) {
...
`;
              ...
              await this.markDocumentSynced(documentId, mergedContentWithId, mergedHash);
              new import_obsidian.Notice(`CoSync: Merged local and remote edits into a single note.`);
            }
          }
        }
        this.bindYjsToEditor();
  ...
  async handleExternalModification(file) {
    ...
    try {
      ...
      const normalizeText = (str) => str.replace(/^\uFEFF/, "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").replace(/\s+/g, " ").trim();
    ...
    if (this.settings.workspaceId === "create-new") {
      try {
        ...
        if (res.ok) {
          ...
          new import_obsidian.Notice(`CoSync: Created new workspace: "${data.name}"`);
        } else {
          console.error("CoSync: Failed to create workspace on server", res.statusText);
          new import_obsidian.Notice("CoSync: Failed to create workspace on server.");
        }
      } catch (err) {
        console.error("CoSync: Error creating workspace on server", err);
        new import_obsidian.Notice("CoSync: Error connecting to server.");
      }
    }
  ...
  async syncVault(isManual = false) {
    ...
    try {
      ...
      const serverDocTitleMap = /* @__PURE__ */ new Map();
      ...
      for (const doc2 of serverDocs) {
        ...
        if (!isMapped) {
          const lowerTitle = doc2.title.toLowerCase();
...
`;
  ...
  async downloadNewDocFromServer(docId, filepath, isMarkdown) {
    const wsUrl = this.settings.serverUrl.replace(/^http/, "ws");
    const roomName = `workspace/${this.settings.workspaceId}/doc/${docId}`;
    const tempYDoc = new Doc();
    const tempWs = new WebsocketProvider(wsUrl, roomName, tempYDoc, {
...
var CoSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Sync Interval (seconds)").setDesc("How often (in seconds) the background synchronization should run.").addText((text2) => text2.setPlaceholder("30").setValue(String(this.plugin.settings.syncInterval)).onChange(async (value) => {
...
function updateYTextCleanly(ytext, newText) {
  ...
  if (deleteCount > 0 || insertText2.length > 0) {
    ytext.delete(commonPrefixLen, deleteCount);
    ytext.insert(commonPrefixLen, insertText2);
  }
}
function applyDiff(ytext, baseText, newText) {
  ...
  let commonSuffixLen = 0;
  const maxSuffixLen = maxLen - commonPrefixLen;
  while (commonSuffixLen < maxSuffixLen && normalizedBase[normalizedBase.length - 1 - commonSuffixLen] === normalizedNew[normalizedNew.length - 1 - commonSuffixLen]) {
    commonSuffixLen++;
  }
  ...
  const insertText2 = normalizedNew.substring(commonPrefixLen, normalizedNew.length - commonSuffixLen);
  if (deleteCount > 0 || insertText2.length > 0) {
    if (deleteCount > 0) {
      ytext.delete(commonPrefixLen, deleteCount);
    }
